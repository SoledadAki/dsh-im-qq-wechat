...
- owner 配对码是临时凭据，只在自己的私聊里使用，不要贴进日志、截图或公开 issue。配对码为一次性高熵串，有有效期与错误次数上限，用尽后自动轮换。
- **绑定的 owner 账号等同于本机的远程操作入口。** 除了派发任务，owner 还能用 `/work <任意绝对路径>` 把工作区指到任意目录，并用 `/safe danger-full-access` 关闭工具审核；两者都不受目录白名单限制。请把通道凭据与配对码视同 SSH 密钥保管，不要绑定到共享或公共账号。
- 只有在需要时才配置通道。未使用的通道也会加载并在启动时拉起对应子进程，减少暴露面就减少 `channels` 列表。
